...
```Shell
csv_to_xlsx.py 入力ファイル名 出力ファイル名 [-s シート名] [-c セル位置] [-h ヘッダ行] [-d データ形式]
　　　　　　　　　　　　　　　　　　　　　　 [-f フォント サイズ] [-u ヘッダ部塗りつぶし色]
　　　　　　　　　　　　　　　　　　　　　　 [-n] [-b] [-w] [-t] [ヘッダ文字列1 文字列2 文字列3 . . . ]
オプション -s sheet_name  　シート名を設定
　　　　　 -c セル位置　　　先頭のセル位置を A1形式で指定
　　　　　 -h ヘッダ行　　　CSVファイルのヘッダ行の位置を指定、先頭行の場合は 1(=default値)、
　　　　　　　　　　　　　　ヘッダ行無しは 0を指定、n (n>1)の場合、n-1 行読み飛ばし
　　　　　　　　　　　　　　ヘッダ行無しの場合、Excelファイルには "col1","col2", ... というヘッダが入力される
　　　　　　　　　　　　　　（後述のオプションで変更可能）
　　　　　 -d データ形式 　データ型をカラム毎に1文字で指定、指定可能な型は S:文字列, I:整数, R:浮動小数, D:日付, V:自動判定
　　　　　 -f フォント名 サイズ　フォント名(英語表記)とサイズ(default: BIZ UDゴシック/BIZ UDPゴシック(ヘッダ) 10ポイント)
　　　　　　　　　　　　　　　　 本オプションのみ 2回指定可 1回目データ部 2回目ヘッダ部
　　　　　 -u 色指定 　　　ヘッダ部分の塗りつぶし色をrgbの16進数(6文字)で指定 (default: 88ccff)
　　　　　 -n　　　　　　　先頭列のデータ番号(1,2,3, . . .)を付加
　　　　　 -b　　　　　　　罫線を設定
　　　　　 -w　　　　　　　セル幅を調整 (列毎の最大文字数でセル幅を設定)
　　　　　 -v　　　　　　　データ型を自動判定 (-d での指定が優先)
　　　　　 -t　　　　　　　デリミタを カンマ から タブ に変更
　　　　　 ヘッダ文字列　　ヘッダの文字列を再設定、オプションの最後尾でのみ設定可能
```
